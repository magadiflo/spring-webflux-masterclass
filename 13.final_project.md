...
Hasta este punto tenemos los tres microservicios completamente implementados. El `stock-service` es el servicio externo
disponible en el directorio `/servers`.

![01.png](assets/trading-platform/01.png)

### 🚀 Levantando los microservicios

Iniciamos los tres servicios de la siguiente manera:

| Microservicio        | Puerto | Método de inicio |
|----------------------|--------|------------------|
| `customer-service`   | `6060` | IDE              |
| `aggregator-service` | `8080` | IDE              |
| `stock-service`      | `7070` | Consola          |

Para levantar el `stock-service` desde la consola:

````bash
D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass (feature/section-13)
...
2026-05-09T23:12:12.809-05:00  INFO 7360 --- [           main] c.v.e.ExternalServicesApplication        : Starting ExternalServicesApplication v0.0.1-SNAPSHOT using Java 25.0.2 with PID 7360 (D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\servers\external-services.jar started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass)
2026-05-09T23:12:12.813-05:00  INFO 7360 --- [           main] c.v.e.ExternalServicesApplication        : No active profile set, falling back to 1 default profile: "default"
...
2026-05-09T23:12:14.868-05:00  INFO 7360 --- [           main] o.s.b.web.embedded.netty.NettyWebServer  : Netty started on port 7070
2026-05-09T23:12:14.886-05:00  INFO 7360 --- [           main] c.v.e.ExternalServicesApplication        : Started ExternalServicesApplication in 2.569 seconds (process running for 3.124)
````

> 📌 **Nota:** Todas las llamadas se realizan a través del `aggregator-service` `(port 8080)`, que actúa como puerta de
> entrada y orquestador.

### ✅ Obtener información de un cliente existente

El cliente existe, el `aggregator-service` delega la consulta al `customer-service` y retorna su información:
saldo actual y portafolio de acciones (vacío en este caso).
...
### ❌ Obtener un cliente con ID inexistente

El `customer-service` responde con `404 Not Found`. El `WebClient` del `aggregator-service` lo intercepta,
lo transforma en una `RemoteCustomerNotFoundException` y el `RestControllerAdvice` construye la respuesta en
...
<
{
  "detail": "El cliente con id [10] no fue encontrado en el customer-service",
...
### ❌ Ejecutar una operación de trade con datos inválidos

Intentamos vender acciones enviando una cantidad negativa `(quantity: -1)`, lo cual viola las reglas de validación del
`aggregator-service`.

Este error es capturado por la validación propia del `aggregator-service` antes de llegar al `customer-service`,
por eso el `status` es `400 Bad Request` directo, no un `502 Bad Gateway`.

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"ticker\": \"GOOGLE\", \"tradeAction\": \"SELL\", \"quantity\": -1}" http://localhost:8080/api/v1/customers/1/trade | jq
>
...
<
{
  ...
  "instance": "/api/v1/customers/1/trade",
  "status": 400,
  "title": "Solicitud de comercio no válida"
}
````

### ❌ Vender acciones sin suficiente stock en el portafolio

El cliente `id=1` no tiene acciones de `GOOGLE`, por lo que no puede venderlas.

El `customer-service` detecta que el cliente no tiene suficientes acciones y responde con `400 Bad Request`.
El `aggregator-service` lo intercepta como un error proveniente de un servicio aguas abajo y lo transforma en
`502 Bad Gateway`, manteniendo el mensaje original del `customer-service` en el campo detail.
...
<
{
  "detail": "Cliente con id [1] no tiene acciones suficientes para completar la transacción",
  "instance": "/api/v1/customers/1/trade",
  "status": 502,
  "title": "Error en servicio remoto"
}
````

### ✅ Comprar 10 acciones de GOOGLE

La operación se ejecuta exitosamente. El `aggregator-service` orquesta la llamada al `stock-service` para obtener
el precio actual de `GOOGLE (price: 7)` y al `customer-service` para registrar la compra. El `balance` del cliente
se actualiza de `10000` a `9930` tras descontar el `totalPrice` de `70`.

````bash
$ curl -v -X POST -H "Content-Type: application/json" -d "{\"ticker\": \"GOOGLE\", \"tradeAction\": \"BUY\", \"quantity\": 10}" http://localhost:8080/api/v1/customers/1/trade | jq
>
...
### ✅ Verificar el portafolio actualizado del cliente

Tras realizar varias operaciones adicionales (hasta acumular `15 acciones de GOOGLE` y comprar `5 de APPLE`).
El portafolio refleja correctamente las operaciones realizadas: 5 acciones de APPLE y 15 de GOOGLE, con un `balance`
actualizado de `9330`.
...
### 📡 Verificar el flujo de precios en tiempo real

El `aggregator-service` expone el `stream` de precios como `text/event-stream (SSE)` hacia los clientes finales,
consumiendo internamente el `application/x-ndjson` del `stock-service`.

Los precios se emiten indefinidamente, uno por cada cambio detectado en el `stock-service`. El prefijo `data:`
es el formato estándar de `SSE (text/event-stream)` que el navegador o cliente final puede consumir directamente.

````bash
$ curl -v http://localhost:8080/api/v1/stock/price-stream
>
< HTTP/1.1 200 OK
< transfer-encoding: chunked
< Content-Type: text/event-stream;charset=UTF-8
<
data:{"ticker":"AMAZON","price":202,"time":"2026-05-09T23:36:10.8814605"}

data:{"ticker":"APPLE","price":80,"time":"2026-05-09T23:36:11.8814429"}

data:{"ticker":"MICROSOFT","price":65,"time":"2026-05-09T23:36:11.8824461"}

data:{"ticker":"GOOGLE","price":29,"time":"2026-05-09T23:36:11.8824461"}

data:{"ticker":"AMAZON","price":199,"time":"2026-05-09T23:36:11.8824461"}

data:{"ticker":"APPLE","price":78,"time":"2026-05-09T23:36:12.8812561"}

data:{"ticker":"MICROSOFT","price":62,"time":"2026-05-09T23:36:12.8812561"}

...
````

## 🧪 Pruebas de Integración a controladores del aggregator-service

### 🔍 ¿Qué tipo de pruebas son?

Antes de ver el código, es importante identificar correctamente el tipo de pruebas que estamos realizando.

Estas **no son pruebas unitarias**, aunque a primera vista pueda parecer que sí porque "mockeamos" dependencias.
La diferencia clave está en **qué tan real es el entorno de prueba**:

| Característica          | Prueba Unitaria          | Prueba de Integración (este caso)                 |
|-------------------------|--------------------------|---------------------------------------------------|
| Contexto de Spring      | ❌ No se levanta          | ✅ Se levanta con `@SpringBootTest`                |
| Servidor HTTP           | ❌ No existe              | ✅ `MockServer` levanta un servidor HTTP real      |
| `WebClient`             | ❌ Mockeado con Mockito   | ✅ Hace peticiones HTTP reales a `MockServer`      |
| Interacción entre capas | ❌ Solo una clase aislada | ✅ Controlador → Servicio → WebClient → MockServer |

En estas pruebas:

- Se levanta el contexto completo de Spring con `@SpringBootTest`.
- `MockServer` levanta un **servidor HTTP real** en un puerto libre que simula el `customer-service` y el
  `stock-service`.
- El `WebClient` del `aggregator-service` hace **peticiones HTTP reales** hacia ese servidor simulado.
- El controlador, el servicio y el cliente HTTP interactúan entre sí de forma real.

Por eso son `pruebas de integración`: prueban la interacción real entre las capas del `aggregator-service`
(controlador → servicio → `WebClient`) contra servicios externos simulados por `MockServer`.

### 📁 Archivos de respuesta simulada en `src/test/resources`

Antes de definir las clases de prueba, necesitamos los archivos JSON que `MockServer` usará como cuerpo de las
respuestas simuladas. Estos archivos representan las respuestas que devolverían el `customer-service` y el
`stock-service` ante distintos escenarios.

### 📂 `src/test/resources/customer-service`

#### `customer-information-200.json`

Respuesta exitosa al consultar la información de un cliente existente.
...
#### `customer-information-404.json`

Respuesta de error cuando el cliente no existe en el `customer-service`.

````json
{
  "detail": "El cliente [id=1] no fue encontrado"
}
````

#### `customer-trade-201.json`

Respuesta exitosa al registrar una operación de trading (compra o venta).
...
Respuesta de error cuando el cliente no tiene fondos suficientes para completar la compra.

````json
{
  "detail": "El cliente [id=1] no tiene fondos suficientes para completar la transacción"
}
````

### 📂 `src/test/resources/stock-service`

#### `stock-price-200.json`

Respuesta exitosa al consultar el precio actual de una acción.
...
#### `stock-price-stream-200.jsonl`

Stream de actualizaciones de precios en formato `NDJSON` (una línea JSON por evento). Se usa para simular el
endpoint `/stock/price-stream` del `stock-service`.

```jsonl
{"ticker": "GOOGLE", "price": 53, "time": "2026-01-01T12:00:01"}
{"ticker": "GOOGLE", "price": 54, "time": "2026-01-01T12:00:02"}
{"ticker": "GOOGLE", "price": 55, "time": "2026-01-01T12:00:03"}
```

> 📌 **Nota:** La extensión `.jsonl` (JSON Lines) indica que cada línea es un objeto JSON independiente,
> que es exactamente el formato `application/x-ndjson` que emite el `stock-service`.

### 🏗️ Clase base: `BaseTest`

Todas las clases de prueba del `aggregator-service` extienden de esta clase abstracta, que centraliza la
configuración común del entorno de pruebas.
...
#### 🔎 Explicación de anotaciones y componentes

#### `@MockServerTest`

Anotación proporcionada por la librería `mockserver-spring-test-listener-no-dependencies`. Al colocarla en la
clase, registra un `Spring TestExecutionListener` que:

- Levanta una instancia de `MockServer` en un **puerto libre** antes de ejecutar las pruebas.
- Registra automáticamente la propiedad `mockServerPort` en el entorno de Spring con el puerto asignado.
- **Reinicia `MockServer`** después de cada prueba, limpiando todas las expectativas configuradas para garantizar
  el aislamiento entre tests.
- **Cierra `MockServer`** automáticamente al finalizar toda la ejecución de pruebas.
- Inyecta automáticamente una instancia de `MockServerClient` en el campo `protected MockServerClient mockServerClient`
  de la clase de prueba, sin necesidad de `@Autowired`.

#### `@AutoConfigureWebTestClient`

Configura automáticamente una instancia de `WebTestClient` en el contexto de Spring. `WebTestClient` es el
cliente HTTP reactivo de pruebas de Spring WebFlux que permite hacer peticiones HTTP y verificar respuestas de
forma fluida. Se inyecta en el campo `protected WebTestClient client`.

#### `@SpringBootTest`

Levanta el contexto completo de Spring para las pruebas. El atributo `properties` permite sobreescribir
propiedades de configuración específicamente para el entorno de pruebas:
...
Aquí ocurre algo importante: **`${mockServerPort}` no es una variable de Spring**, sino una propiedad que
`@MockServerTest` registra automáticamente en el entorno de Spring cuando levanta `MockServer`. Es decir:

1. `@MockServerTest` inicia `MockServer` en un puerto libre (ej: `51234`).
2. Registra `mockServerPort=51234` en el entorno de Spring.
3. `@SpringBootTest` resuelve `${mockServerPort}` como `51234`.
4. Las URLs base del `customer-service` y `stock-service` quedan apuntando a `http://localhost:51234`,
   que es donde `MockServer` está escuchando.

> 📌 **Nota:** Ambos servicios externos (`customer-service` y `stock-service`) apuntan al **mismo puerto**
> de `MockServer`. Esto es posible porque `MockServer` diferencia las peticiones por su `path` (ruta),
> no por el puerto. Así, con una sola instancia de `MockServer` podemos simular múltiples servicios externos.

#### `protected MockServerClient mockServerClient`

Cliente que permite configurar las **expectativas** de `MockServer` desde las pruebas. Una expectativa define:
qué petición esperar y qué respuesta devolver. Es inyectado automáticamente por `@MockServerTest` sin necesidad
de ninguna anotación adicional.

#### `private static final Path TEST_RESOURCES_PATH`

Ruta base hacia el directorio `src/test/resources` donde están almacenados los archivos JSON de respuestas
simuladas.

#### `@BeforeAll` → `ConfigurationProperties.disableLogging(true)`

Se ejecuta una sola vez antes de todos los tests de la clase. Deshabilita los logs internos de `MockServer`
para mantener la salida de consola limpia durante la ejecución de las pruebas.

#### `resourceToString(String relativePath)`

Método utilitario que lee el contenido de un archivo del directorio `src/test/resources` y lo retorna como
`String`. Es usado por las subclases para cargar los JSON de respuestas simuladas al configurar las
expectativas de `MockServer`.

### 🧪 Clase de prueba: `CustomerPortfolioControllerTest`

Prueba los endpoints del controlador responsable del portafolio de clientes en el `aggregator-service`.
...
#### Estructura de cada prueba: patrón `given / when / then`

Cada prueba sigue el patrón **Given / When / Then (GWT)**, una estructura muy utilizada en pruebas de integración y
pruebas orientadas al comportamiento (BDD).

- `given` → Configura la expectativa en `MockServer`: qué petición esperar y qué respuesta devolver.

- `when` → Ejecuta la acción que se desea probar.  
  Aquí se realiza una petición HTTP al controlador del `aggregator-service` utilizando `WebTestClient`.

- `then` → Verifica el resultado esperado.  
  Se valida el estado HTTP, el cuerpo de la respuesta y cualquier comportamiento esperado del endpoint.

#### `mockCustomerInformation(String path, int responseCode)`

Método privado que configura una **expectativa** en `MockServer`.
...
> ⚠️ **Aclaración importante sobre el path `/api/v1/customers/1`:**
>
> Este path **no es el endpoint del controlador del `aggregator-service`**. Es la ruta que el
> `aggregator-service` usa internamente para llamar al `customer-service` a través del `WebClient`.
>
> El flujo es el siguiente:
> ```
> WebTestClient → [GET /api/v1/customers/1] → Controlador del aggregator-service
>                                                       ↓
>                                         WebClient llama internamente a:
>                                         [GET /api/v1/customers/1] → MockServer
>                                                       ↓
>                                         MockServer responde con el JSON configurado
> ```
>
> Casualmente ambos paths son iguales en este ejemplo, pero son peticiones completamente distintas:
> una va al controlador del `aggregator-service` y la otra va al `MockServer` que simula al
> `customer-service`.

- `.when(HttpRequest.request("/api/v1/customers/1"))` → define qué petición interceptar.
- `.respond(...)` → define qué respuesta devolver cuando se intercepte esa petición.
- `.withStatusCode(responseCode)` → define el código HTTP de la respuesta simulada.
- `.withContentType(MediaType.APPLICATION_JSON)` → define el `Content-Type` de la respuesta.

#### `getCustomerInformation(HttpStatus expectedStatus)`

Método privado que ejecuta la petición al **controlador del `aggregator-service`** usando `WebTestClient`
y verifica el status HTTP esperado.
...
Escenario exitoso: el `customer-service` existe y responde con `200 OK`.

1. `MockServer` es configurado para responder con el contenido de `customer-information-200.json` y
   status `200` cuando reciba `GET /api/v1/customers/1`.
2. `WebTestClient` llama al controlador del `aggregator-service` en `GET /api/v1/customers/1`.
3. El controlador internamente llama al `customer-service` (que es `MockServer`).
4. `MockServer` responde con el JSON configurado.
5. El controlador retorna la respuesta al `WebTestClient`.
6. Se verifican los campos del JSON retornado con `jsonPath`.

#### Test: `customerNotFound()`

Escenario de error: el `customer-service` responde con `404 Not Found`.

1. `MockServer` es configurado para responder con el contenido de `customer-information-404.json` y
   status `404`.
2. `WebTestClient` llama al controlador del `aggregator-service`.
3. El `WebClient` intercepta el `404`, lo transforma en `RemoteCustomerNotFoundException` y el
   `RestControllerAdvice` construye una respuesta `ProblemDetail`.
4. Se verifica que la respuesta final contiene los campos del `ProblemDetail` correctamente construidos
   por el `ApplicationExceptionHandler`.

> 📌 **Nota:** Observa que el `detail` verificado en el test
> (`"El cliente con id [1] no fue encontrado en el customer-service"`) **no es exactamente el mismo**
> que el que viene en `customer-information-404.json`
> (`"El cliente [id=1] no fue encontrado"`). Esto es porque el `aggregator-service` construye su propio
> mensaje a través de la excepción `RemoteCustomerNotFoundException`, que tiene su propio formato de mensaje
> definido en el constructor de la excepción.

#### Ejecución de la prueba

````bash
D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service (feature/section-13)
$ mvn test -Dtest="CustomerPortfolioControllerTest"
[INFO] Scanning for projects...
[INFO]
[INFO] ------------------< dev.magadiflo:aggregator-service >------------------
[INFO] Building aggregator-service 0.0.1-SNAPSHOT
[INFO]   from pom.xml
[INFO] --------------------------------[ jar ]---------------------------------
[INFO]
...
...
[INFO] -------------------------------------------------------
...
2026-05-12T11:12:59.300-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioControllerTest  : Starting CustomerPortfolioControllerTest using Java 25.0.2 with PID 7584 (started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service)
2026-05-12T11:12:59.302-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioControllerTest  : No active profile set, falling back to 1 default profile: "default"
2026-05-12T11:12:59.904-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:49405
2026-05-12T11:13:00.082-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:49405
2026-05-12T11:13:00.445-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioControllerTest  : Started CustomerPortfolioControllerTest in 1.471 seconds (process running for 2.924)
...
2026-05-12T11:13:01.861-05:00  INFO 7584 --- [aggregator-service] [     parallel-1] d.m.a.app.client.CustomerServiceClient   : [customer-service] Iniciando solicitud de información del cliente con id: 1
2026-05-12T11:13:02.122-05:00  INFO 7584 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [Remote: customer-service][Op: GET][Type: CustomerInformation] Respuesta exitosa para cliente con ID: 1
2026-05-12T11:13:02.257-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioControllerTest  : {"id":1,"name":"sam","balance":10000,"holdings":[{"ticker":"GOOGLE","quantity":2}]}
2026-05-12T11:13:02.340-05:00  INFO 7584 --- [aggregator-service] [     parallel-2] d.m.a.app.client.CustomerServiceClient   : [customer-service] Iniciando solicitud de información del cliente con id: 1
2026-05-12T11:13:02.356-05:00  WARN 7584 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [Remote: customer-service][Op: GET][Type: CustomerInformation] Cliente no encontrado con ID: 1. Status original: 404 NOT_FOUND - Detalle: El cliente [id=1] no fue encontrado
2026-05-12T11:13:02.362-05:00 ERROR 7584 --- [aggregator-service] [ctor-http-nio-2] d.m.a.a.a.ApplicationExceptionHandler    : RemoteCustomerNotFoundException: El cliente con id [1] no fue encontrado en el customer-service
2026-05-12T11:13:02.371-05:00  INFO 7584 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioControllerTest  : {"detail":"El cliente con id [1] no fue encontrado en el customer-service","instance":"/api/v1/customers/1","status":404,"title":"Cliente no encontrado"}
[INFO] Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 4.149 s -- in dev.magadiflo.aggregator.app.CustomerPortfolioControllerTest
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2026-05-12T11:13:02-05:00
[INFO] ------------------------------------------------------------------------
````

### 🧪 Clase de prueba: `CustomerPortfolioController2Test`

Prueba el endpoint `POST /api/v1/customers/{customerId}/trade` del `aggregator-service`, que orquesta llamadas tanto
al `stock-service` (para obtener el precio actual) como al `customer-service` (para registrar la operación).
...
#### `mockCustomerTrade(String customerServiceFilePath, int responseCode)`

Método privado que configura las expectativas de ambos servicios externos en `MockServer`, ya que el endpoint de `trade`
orquesta dos llamadas internas:

El orden importa: primero el `aggregator-service` consulta el precio al `stock-service`, luego envía ese precio junto
con los datos del `trade` al `customer-service`.

Un detalle clave está en `.withBody(RegexBody.regex(".*\"price\":110.*"))`: no validamos todo el cuerpo del request,
solo que contenga `"price":110`. Esto garantiza que el `aggregator-service` efectivamente consultó el precio al
`stock-service` y lo incluyó correctamente en la petición al `customer-service`. Si ese campo no estuviera presente,
`MockServer` no encontraría ninguna expectativa que coincida y respondería con `404`.

#### Test: `tradeSuccess()`

Escenario exitoso: el `customer-service` responde con `201 Created`.

`MockServer` simula una respuesta exitosa del `customer-service` con el contenido de `customer-trade-201.json`.
Se verifica que el balance y el precio total calculado sean correctos.

#### Test: `tradeFailure()`

Escenario de error: el `customer-service` responde con `400 Bad Request` (fondos insuficientes).

`MockServer` simula la respuesta de error con el contenido de `customer-trade-400.json`. El `WebClient` del
`aggregator-service` intercepta el `400`, lo transforma en `RemoteClientException` y el `RestControllerAdvice`
construye la respuesta final como `502 Bad Gateway`.

#### Test: `inputValidation()`

Escenario de validación propia del `aggregator-service`, sin llamada a ningún servicio externo. Se prueban tres
casos de entrada inválida en un mismo test:

| Request inválido    | Error esperado                  |
|---------------------|---------------------------------|
| `ticker: null`      | `"El Ticker es requerido"`      |
| `tradeAction: null` | `"El TradeAction es requerido"` |
| `quantity: -2`      | `"La cantidad debe ser > 0"`    |

> 📌 Estos errores son capturados por la validación propia del `aggregator-service` antes de llegar a cualquier
> servicio externo, por eso el `status` es `400 Bad Request` directo y no un `502 Bad Gateway`.

#### Ejecución de la prueba

````bash
D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service (feature/section-13)
$ mvn test -Dtest="CustomerPortfolioController2Test"
[INFO] Scanning for projects...
[INFO]
[INFO] ------------------< dev.magadiflo:aggregator-service >------------------
[INFO] Building aggregator-service 0.0.1-SNAPSHOT
...
[INFO]  T E S T S
[INFO] -------------------------------------------------------
[INFO] Running dev.magadiflo.aggregator.app.CustomerPortfolioController2Test
11:14:44.373 [main] INFO org.springframework.test.context.support.AnnotationConfigContextLoaderUtils -- Could not detect default configuration classes for test class [dev.magadiflo.aggregator.app.CustomerPortfolioController2Test]: CustomerPortfolioController2Test does not declare any static, non-private, non-final, nested classes annotated with @Configuration.
11:14:44.850 [main] INFO org.springframework.boot.test.context.SpringBootTestContextBootstrapper -- Found @SpringBootConfiguration dev.magadiflo.aggregator.app.AggregatorServiceApplication for test class dev.magadiflo.aggregator.app.CustomerPortfolioController2Test
11:14:44.950 [main] INFO org.springframework.test.context.support.AnnotationConfigContextLoaderUtils -- Could not detect default configuration classes for test class [dev.magadiflo.aggregator.app.CustomerPortfolioController2Test]: CustomerPortfolioController2Test does not declare any static, non-private, non-final, nested classes annotated with @Configuration.
11:14:44.951 [main] INFO org.springframework.test.context.support.AnnotationConfigContextLoaderUtils -- Could not detect default configuration classes for test class [dev.magadiflo.aggregator.app.BaseTest]: BaseTest does not declare any static, non-private, non-final, nested classes annotated with @Configuration.
11:14:44.956 [main] INFO org.springframework.boot.test.context.SpringBootTestContextBootstrapper -- Found @SpringBootConfiguration dev.magadiflo.aggregator.app.AggregatorServiceApplication for test class dev.magadiflo.aggregator.app.CustomerPortfolioController2Test
...
2026-05-12T11:14:45.405-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : Starting CustomerPortfolioController2Test using Java 25.0.2 with PID 17800 (started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service)
2026-05-12T11:14:45.407-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : No active profile set, falling back to 1 default profile: "default"
2026-05-12T11:14:46.188-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:49651
2026-05-12T11:14:46.376-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:49651
2026-05-12T11:14:46.764-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : Started CustomerPortfolioController2Test in 1.693 seconds (process running for 3.51)
...
2026-05-12T11:14:48.043-05:00 ERROR 17800 --- [aggregator-service] [     parallel-1] d.m.a.a.a.ApplicationExceptionHandler    : InvalidTradeRequestException: El Ticker es requerido
2026-05-12T11:14:48.083-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : {"detail":"El Ticker es requerido","instance":"/api/v1/customers/1/trade","status":400,"title":"Solicitud de comercio no válida"}
2026-05-12T11:14:48.113-05:00 ERROR 17800 --- [aggregator-service] [     parallel-2] d.m.a.a.a.ApplicationExceptionHandler    : InvalidTradeRequestException: El TradeAction es requerido
2026-05-12T11:14:48.114-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : {"detail":"El TradeAction es requerido","instance":"/api/v1/customers/1/trade","status":400,"title":"Solicitud de comercio no válida"}
2026-05-12T11:14:48.117-05:00 ERROR 17800 --- [aggregator-service] [     parallel-3] d.m.a.a.a.ApplicationExceptionHandler    : InvalidTradeRequestException: La cantidad debe ser > 0
2026-05-12T11:14:48.118-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : {"detail":"La cantidad debe ser > 0","instance":"/api/v1/customers/1/trade","status":400,"title":"Solicitud de comercio no válida"}
2026-05-12T11:14:48.822-05:00  INFO 17800 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [customer-service] Iniciando solicitud de trade para cliente id: 1 [BUY GOOGLE]
2026-05-12T11:14:48.837-05:00  INFO 17800 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [Remote: customer-service][Op: POST][Type: StockTradeResponse] Respuesta exitosa para cliente con ID: 1
2026-05-12T11:14:48.846-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : {"customerId":1,"ticker":"GOOGLE","quantity":2,"price":110,"tradeAction":null,"totalPrice":220,"balance":9780}2026-05-12T11:14:48.877-05:00  INFO 17800 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [customer-service] Iniciando solicitud de trade para cliente id: 1 [BUY GOOGLE]
2026-05-12T11:14:48.893-05:00  WARN 17800 --- [aggregator-service] [ctor-http-nio-2] d.m.a.app.client.CustomerServiceClient   : [Remote: customer-service][Op: POST][Type: StockTradeResponse] Servicio externo respondió con error de cliente. Status original: 400 BAD_REQUEST - Detalle: El cliente [id=1] no tiene fondos suficientes para completar la transacción
2026-05-12T11:14:48.895-05:00 ERROR 17800 --- [aggregator-service] [ctor-http-nio-2] d.m.a.a.a.ApplicationExceptionHandler    : RemoteClientException: El cliente [id=1] no tiene fondos suficientes para completar la transacción
2026-05-12T11:14:48.896-05:00  INFO 17800 --- [aggregator-service] [           main] d.m.a.a.CustomerPortfolioController2Test : {"detail":"El cliente [id=1] no tiene fondos suficientes para completar la transacción","instance":"/api/v1/customers/1/trade","status":502,"title":"Error en servicio remoto"}
[INFO] Tests run: 3, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 4.869 s -- in dev.magadiflo.aggregator.app.CustomerPortfolioController2Test
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2026-05-12T11:14:49-05:00
[INFO] ------------------------------------------------------------------------
````

### 🧪 Clase de prueba: `StockPriceStreamControllerTest`

Prueba el endpoint de streaming `GET /api/v1/stock/price-stream` del `aggregator-service`, que consume el stream
`application/x-ndjson` del `stock-service` y lo expone como `text/event-stream (SSE)` hacia los clientes finales.
...
Este test tiene dos particularidades importantes que lo diferencian de los anteriores:

1. La verificación es reactiva con `StepVerifier`  
   A diferencia de los tests anteriores donde usábamos `jsonPath` para verificar una respuesta única, aquí el endpoint
   retorna un stream continuo de eventos. Por eso se usa `StepVerifier` de Project Reactor, que permite verificar
   elementos de un `Flux` de forma secuencial y reactiva.
2. La negociación de `MediaType` refleja exactamente la arquitectura del sistema. Donde:
    ````
    // MockServer simula el stock-service con application/x-ndjson (backend a backend)
    .withContentType(MediaType.parse("application/x-ndjson"))
    
    // WebTestClient consume el aggregator-service como text/event-stream (cliente a servidor)
    .accept(org.springframework.http.MediaType.TEXT_EVENT_STREAM)
    ````
   Esto refleja exactamente lo que vimos al diseñar el `StockServiceClient`: el `aggregator-service` consume
   `application/x-ndjson` del `stock-service` internamente, pero expone el stream como `text/event-stream` hacia los
   clientes finales.

Flujo completo del test:

````
WebTestClient                aggregator-service              MockServer
     │                               │                             │
     │ GET /api/v1/stock/price-stream│                             │
...
> 📌 Los tres eventos verificados (price: 53, 54, 55) corresponden exactamente a las tres líneas definidas en
> `stock-price-stream-200.jsonl`. El `.verifyComplete()` confirma que el stream terminó correctamente tras emitir
> los tres elementos.
...
````bash
D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service (feature/section-13)
$ mvn test -Dtest="StockPriceStreamControllerTest"
[INFO] Scanning for projects...
[INFO]
[INFO] ------------------< dev.magadiflo:aggregator-service >------------------
[INFO] Building aggregator-service 0.0.1-SNAPSHOT
...
[INFO] -------------------------------------------------------
...
2026-05-12T11:53:49.990-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : Starting StockPriceStreamControllerTest using Java 25.0.2 with PID 11248 (started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\trading-platform\aggregator-service)
2026-05-12T11:53:49.992-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : No active profile set, falling back to 1 default profile: "default"
2026-05-12T11:53:50.588-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:53333
2026-05-12T11:53:50.861-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.app.config.ServiceClientConfig     : baseUrl: http://localhost:53333
2026-05-12T11:53:51.368-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : Started StockPriceStreamControllerTest in 1.672 seconds (process running for 3.141)
...
2026-05-12T11:53:53.607-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : PriceUpdate[ticker=GOOGLE, price=53, time=2026-01-01T12:00:01]
2026-05-12T11:53:53.659-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : PriceUpdate[ticker=GOOGLE, price=54, time=2026-01-01T12:00:02]
2026-05-12T11:53:53.660-05:00  INFO 11248 --- [aggregator-service] [           main] d.m.a.a.StockPriceStreamControllerTest   : PriceUpdate[ticker=GOOGLE, price=55, time=2026-01-01T12:00:03]
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 4.767 s -- in dev.magadiflo.aggregator.app.StockPriceStreamControllerTest
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2026-05-12T11:53:54-05:00
[INFO] ------------------------------------------------------------------------
````

## 🧪 Prueba final del proyecto: Plataforma de negociación (Frontend)

- Creamos un archivo `index.html` y lo colocamos en la siguiente ruta `src/main/resources/static`.
- El archivo `index.html` actúa como interfaz web para consumir los endpoints del `aggregator-service`.
- La lógica JavaScript conecta los elementos del DOM con los servicios REST y el stream SSE.
...
<head>
    ...
    <!-- Evita la búsqueda automática de favicon.ico -->
...
<div class="container-lg">
    ...
    <div class="row mt-3">
        <div class="col-sm-3">
            <div class="card">
                ...
                <div class="card-body">
                    <h5 class="card-title text-center" id="APPLE">$100</h5>
                    <div class="row">
                        <div class="col" align="left"><a href="#" class="btn btn-success"
                                                         onclick="trade('APPLE', 'BUY')">Buy</a></div>
                        <div class="col" align="right"><a href="#" class="btn btn-warning"
                                                          onclick="trade('APPLE', 'SELL')">Sell</a></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-3">
            <div class="card">
                ...
                <div class="card-body">
                    <h5 class="card-title text-center" id="AMAZON">$100</h5>
                    <div class="row">
                        <div class="col" align="left"><a href="#" class="btn btn-success"
                                                         onclick="trade('AMAZON', 'BUY')">Buy</a></div>
                        <div class="col" align="right"><a href="#" class="btn btn-warning"
                                                          onclick="trade('AMAZON', 'SELL')">Sell</a></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-3">
            <div class="card">
                ...
                <div class="card-body">
                    <h5 class="card-title text-center" id="GOOGLE">$100</h5>
                    <div class="row">
                        <div class="col" align="left"><a href="#" class="btn btn-success"
                                                         onclick="trade('GOOGLE', 'BUY')">Buy</a></div>
                        <div class="col" align="right"><a href="#" class="btn btn-warning"
                                                          onclick="trade('GOOGLE', 'SELL')">Sell</a></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-sm-3">
            <div class="card">
                ...
                <div class="card-body">
                    <h5 class="card-title text-center" id="MICROSOFT">$100</h5>
                    <div class="row">
                        <div class="col" align="left"><a href="#" class="btn btn-success"
                                                         onclick="trade('MICROSOFT', 'BUY')">Buy</a></div>
                        <div class="col" align="right"><a href="#" class="btn btn-warning"
                                                          onclick="trade('MICROSOFT', 'SELL')">Sell</a></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row mt-3">
        ...
        <div class="col-sm-3">
            ...
            <div class="card w-100">
                ...
                <div class="card-body">
                    <img src="data:image/jpeg;base64,"
    ...
    <script>
        ...
        const stockPriceUpdates = () => {
            var source = new EventSource("/api/v1/stock/price-stream");
            source.onmessage = (evt) => {
                ...
                if (customerQuantity) {
                    ...
                    document.getElementById(`customer-value-${event.ticker}`).innerText = `$${price}`;
        ...
        const trade = async (ticker, tradeAction) => {
            const response = await fetch(`/api/v1/customers/${customerId}/trade`, {
                method: "POST",
                headers: {
...
- **Obtención de parámetros**
    - Se obtiene el `customerId` desde la URL `(?customer=1)`.
    - Esto permite que la página cargue el perfil y portafolio del cliente específico.
- **Stream de precios SSE**
    - `EventSource("/api/v1/stock/price-stream")` abre una conexión `SSE`.
    - Cada evento recibido actualiza dinámicamente el precio en la card correspondiente (`APPLE`, `GOOGLE`, etc.).
    - Si el cliente tiene acciones de ese ticker, se recalcula el valor en la tabla del `portafolio` y el
      `total del portafolio`.
- **Carga de información del cliente**
    - `fetchCustomerInformation()` hace un `GET /api/v1/customers/{id}`.
    - Actualiza nombre, balance y holdings en la tabla.
    - Calcula el valor del portafolio sumando `balance + valor de acciones`.
    - Si el total ≥ 10000 se muestra en verde, si es menor se muestra en rojo.
- **Operaciones de trading**
    - Al hacer clic en `Buy` o `Sell`, se ejecuta `trade(ticker, tradeAction)`.
    - Envía un `POST /api/v1/customers/{id}/trade` con `quantity = 1`.
    - Si la respuesta es exitosa `(201 Created)`, se refresca el perfil del cliente.
    - Si ocurre un error `(400, 404, 502)`, se muestra un alert con el detalle `(ProblemDetail.detail)`.
- **Manejo de alertas**
    - `showAlert(msg, level)` crea un mensaje Bootstrap (alert-danger, alert-success).
    - El mensaje desaparece automáticamente después de 3 segundos.
- **Inicialización**
    - Al cargar la página `(DOMContentLoaded)`, se disparan:
        - `stockPriceUpdates()` → inicia el stream `SSE`.
        - `fetchCustomerInformation()` → carga el perfil inicial del cliente.

#### ✅ Conclusión

Con esta interfaz:

- Los precios se actualizan en tiempo real gracias al SSE.
- El cliente puede comprar o vender acciones con un clic.
- El balance y portafolio se recalculan automáticamente tras cada operación.
- Los errores se muestran como alertas claras en pantalla.
- Se logra una experiencia completa de plataforma de negociación interactiva, conectando frontend y backend de forma
  reactiva.

### 🚀 Ejecución final del proyecto: Plataforma de negociación

Con el archivo `index.html` ubicado en `src/main/resources/static/`, reiniciamos todos los microservicios para
iniciar desde cero.
...
- Ejecutado desde consola con `java -jar servers/external-services.jar`.
- Se inicia en el puerto `7070` con Netty embebido.
- Logs confirman arranque exitoso en ~2.5 segundos.
...
2026-05-12T12:13:40.243-05:00  INFO 180 --- [           main] c.v.e.ExternalServicesApplication        : Starting ExternalServicesApplication v0.0.1-SNAPSHOT using Java 25.0.2 with PID 180 (D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass\servers\external-services.jar started by magadiflo in D:\programming\spring\01.udemy\03.vinoth_selvaraj\spring-webflux-masterclass)
2026-05-12T12:13:40.247-05:00  INFO 180 --- [           main] c.v.e.ExternalServicesApplication        : No active profile set, falling back to 1 default profile: "default"
WARNING: A terminally deprecated method in sun.misc.Unsafe has been called
WARNING: sun.misc.Unsafe::objectFieldOffset has been called by io.netty.util.internal.PlatformDependent0$4 (jar:nested:/D:/programming/spring/01.udemy/03.vinoth_selvaraj/spring-webflux-masterclass/servers/external-services.jar/!BOOT-INF/lib/netty-common-4.1.101.Final.jar!/)
WARNING: Please consider reporting this to the maintainers of class io.netty.util.internal.PlatformDependent0$4
WARNING: sun.misc.Unsafe::objectFieldOffset will be removed in a future release
2026-05-12T12:13:42.240-05:00  INFO 180 --- [           main] o.s.b.web.embedded.netty.NettyWebServer  : Netty started on port 7070
2026-05-12T12:13:42.259-05:00  INFO 180 --- [           main] c.v.e.ExternalServicesApplication        : Started ExternalServicesApplication in 2.492 seconds (process running for 3.092)
````

#### ▶️ `customer-service`

- Levantado desde el IDE IntelliJ IDEA.
- Puerto `6060`.
...
- Con los tres servicios activos, accedemos a: `http://localhost:8080/?customer=1`
- La página renderizada corresponde al `index.html` que construimos:
    - Cards con precios de acciones (`APPLE`, `GOOGLE`, `AMAZON`, `MICROSOFT`).
    - Perfil del cliente con `balance` y `valor de portafolio`.
    - Tabla dinámica con holdings.
- **Actualización de precios en tiempo real:**
    - Los valores cambian constantemente gracias al stream `SSE` `(/api/v1/stock/price-stream)`.
    - Cada navegador conectado recibe las mismas actualizaciones de precios.

![04.png](assets/trading-platform/04.png)

### 🔎 Detalles importantes

El parámetro customer en la URL `(?customer=1)` se captura en el script:
...
- Esto permite que la interfaz consulte al backend por el cliente específico.
- Al abrir **dos navegadores distintos** con clientes diferentes, ambos reciben el mismo flujo de precios en
  tiempo real, pero cada uno muestra su propio `balance` y `portafolio`.

![05.png](assets/trading-platform/05.png)

#### ✅ Conclusión

Con esta prueba final:

- Confirmamos que la interfaz web se integra perfectamente con los microservicios.
- Los precios se actualizan en tiempo real vía SSE.
- Cada cliente mantiene su propio estado de portafolio, pero comparte el mismo stream de precios.
- El proyecto queda completo: `backend reactivo + frontend interactivo` funcionando en conjunto.

### 🧪 Caso práctico: Compra de APPLE por Sam

`Sam` realiza una compra de `APPLE`.

![06.png](assets/trading-platform/06.png)

#### 🔎 Análisis de resultados tras compra de APPLE

- **Acción APPLE**
    - Precio al momento de la compra: `$103`.
    - Se ejecuta un `POST /api/v1/customers/1/trade` con `BUY` y `quantity = 1`.
    - El backend descuenta el costo de la compra del balance y agrega la acción al portafolio.
- **Available Balance**
    - Inicialmente: `$10000`.
    - Balance después de la compra: `$10,000` − `$103` = `$9,897`
    - Esto refleja que Sam gastó $103 en adquirir 1 acción de APPLE.
  > 🔎 **¿Por qué viste `$9,895` y `$9,998` en la interfaz?**
  >
  > El balance y el portfolio value se recalculan en tiempo real, pero el precio de APPLE está cambiando constantemente
  > por el stream SSE.
  >
  > Es probable que justo en el instante de la compra el precio de APPLE haya variado (ej. bajó a `$101` o subió a
  > `$105`), y por eso el cálculo que ves en pantalla no coincide exactamente con la resta estática que hicimos.
  >
  > El frontend siempre toma el precio más reciente del SSE, no el precio que estaba en el momento exacto de la compra.
  > Por eso el `Current Value` y el `Portfolio Value` se mueven constantemente.

- **Portfolio Value**
    - Se recalcula como: `balance + valor de holdings`.
    - Holdings ahora incluyen 1 acción de APPLE × `$103` = `$103`.
    - Total: `$9895` + `$103` = `$9998`.
    - Se muestra en rojo porque el valor total es `< 10000` (la lógica del frontend colorea verde si ≥ 10000,
      rojo si < 10000).
    - Este valor se actualiza constantemente porque depende del precio en tiempo real de APPLE que llega vía SSE.
- **Tabla Portfolio**
    - Se agrega una fila:
        - Stock = APPLE
        - Quantity = 1
        - Current Value = `$103`
    - El campo `Current Value` se recalcula cada vez que llega un nuevo precio de APPLE desde el stream SSE.
    - Por eso ves que se “actualiza constantemente”: el frontend multiplica la cantidad (1) × precio actual recibido.

### ✅ Conclusión corregida

- El `Available Balance` baja correctamente según el costo de la compra.
- El `Portfolio Value` refleja `balance + valor de holdings`, pero como el precio de APPLE se actualiza en tiempo real,
  el total fluctúa.
- El `current value` de APPLE se actualiza en tiempo real gracias al SSE, lo que a su vez recalcula el `portfolio value`
  dinámicamente.
- El desfase que notaste (`9895` vs `9897`, `9998` vs `10000`) se debe a que el SSE envió un nuevo precio
