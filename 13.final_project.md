...
En el `aggregator-service`, necesitamos manejar errores de manera consistente y expresiva. Para ello definimos
excepciones específicas del dominio y una clase utilitaria que nos permite integrarlas fácilmente en flujos reactivos.
...
- Se lanza cuando el **cliente solicitado no existe en el sistema.**
- El mensaje incluye el `customerId` para mayor claridad en el error.
...
- El mensaje es dinámico, permitiendo expresar distintas causas (ejemplo: cantidad inválida, ticker faltante, etc.).
...
- **Centralización de errores** → La clase `BusinessErrors` agrupa métodos estáticos para generar errores de negocio
  de forma uniforme.
- **Uso de genéricos `<T>`** → Permite integrar los errores en cualquier flujo reactivo `(Mono<T>)`,
  sin importar el tipo de dato esperado.
- **Mensajes claros y específicos** → Cada método devuelve un error con un mensaje descriptivo, facilitando la
  depuración y mejorando la experiencia del cliente.
