...
- `server.port` → define el puerto de ejecución de la aplicación.
- `error.include-message: always` → asegura que los mensajes de error se incluyan en las respuestas HTTP.
- `spring.r2dbc` → configuración de la cadena de conexión `R2DBC` hacia `PostgreSQL`.
- `spring.webflux.api-version` → habilita el versionamiento de APIs en `Spring Boot 4` para el stack reactivo:
    - `required: true` → obliga a especificar una versión en las llamadas.
    - `supported: 1,2` → define las versiones soportadas.
    - `use.path-segment: 1` → indica que la versión de la API se especifica dentro del **segmento de la URL**.  
      Por ejemplo: `/api/{versión}/customers`. Spring Boot agrega automáticamente el prefijo `v`, por lo que la ruta
      final quedará como:
        - `/api/v1/customers` → si la `{version}` es 1.
        - `/api/v2/customers` → si la `{version}` es 2.
- `logging.level` → activa logs detallados de consultas y parámetros enviados a PostgreSQL.
- `section: sec03` → indica la sección activa del playground.

> **📌 Nota:** Spring Boot ofrece dos stacks web incompatibles entre sí. La configuración de versionamiento de API
> varía según el stack que uses:
> - `spring.mvc.api-version` → para proyectos con **Spring Web (MVC)**, basado en el modelo bloqueante tradicional.
> - `spring.webflux.api-version` → para proyectos con **Spring WebFlux**, basado en el modelo reactivo y no bloqueante.
>
> Usar la clave incorrecta no pasa desapercibido — Spring falla en el arranque con un error como el siguiente:
> ```
> Caused by: org.springframework.beans.factory.BeanCreationException: Error creating bean with name
> 'requestMappingHandlerMapping': API version specified, but no ApiVersionStrategy configured
> Caused by: java.lang.IllegalStateException: API version specified, but no ApiVersionStrategy configured
> ```
> Esto facilita detectar el problema de inmediato.
...
- `properties = "section=sec03"` sobrescribe la propiedad `section` solo para este test, permitiendo ejecutar la
  configuración correspondiente a la sección (paquete) `sec03` sin modificar el `application.properties` global.
...
## 📦 Creando entidad y repositorio para CustomerOrder

### 🧩 Entidad `CustomerOrder`

La entidad `CustomerOrder` mapea a la tabla `customer_orders`:
...
- `UUID` es el tipo correcto para la clave primaria definida en la tabla.
- `Instant` representa un instante en `UTC` y se mapea de forma precisa a `TIMESTAMP WITH TIME ZONE` en `PostgreSQL`.
- Las claves foráneas (`customerId`, `productId`) permiten relacionar órdenes con clientes y productos.

### 📂 Repositorio CustomerOrderRepository

Definimos el repositorio extendiendo de `ReactiveCrudRepository`:
...
- Al usar `ReactiveCrudRepository<CustomerOrder, UUID>` obtenemos métodos CRUD reactivas (`findAll`, `findById`,
  `save`, `deleteById`, etc.).
- El tipo de la clave primaria `(UUID)` debe coincidir con el definido en la entidad.

### ✅ Conclusión

- La entidad `CustomerOrder` refleja correctamente la estructura de la tabla `customer_orders`.
- El repositorio nos permite realizar operaciones CRUD reactivas sobre las órdenes.
- Con esto ya tenemos listas las tres entidades principales (`Customer`, `Product`, `CustomerOrder`) y sus repositorios,
  lo que nos permitirá empezar a trabajar con relaciones y consultas más avanzadas.

## 🔍 Consultas Complejas en Spring Data R2DBC

En ciertos casos, los métodos derivados del nombre de los atributos no son suficientes para expresar consultas
complejas, especialmente cuando involucran múltiples tablas, condiciones específicas o agregaciones.

Para resolver estos escenarios, `Spring Data R2DBC` ofrece dos alternativas principales para escribir
`consultas SQL personalizadas`:

1. Usar la anotación `@Query` directamente en el repositorio.
2. Utilizar el `DatabaseClient` para ejecutar consultas de manera programática.

### 1️⃣ Utilizando `@Query` en los repositorios

Esta opción permite definir directamente una consulta SQL en el repositorio, asociándola a un método. Es ideal para
mantener una estructura declarativa y sencilla.

#### ✅ Ventajas

- Permite usar **SQL puro**, lo cual da mayor control y eficiencia.
- Evita problemas como el **N+1**, ya que se pueden usar `JOINs` explícitos.
- Facilita mantener las consultas cerca de los métodos del repositorio.

### 🧩 Ejemplo 1: Consultas complejas con `@Query` retornando entidades completas

Supongamos que queremos obtener todos los productos que ha ordenado un cliente según su nombre. Podemos hacerlo con una
consulta SQL usando `@Query`:
...
Creamos la clase `Lec03CustomerOrderRepositoryTest` para validar la consulta:
...
El log muestra el parámetro enviado y la consulta SQL ejecutada:

````bash
2026-03-18T11:13:38.214-05:00 DEBUG 15220 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.PARAM                : Bind parameter [0] to: mike
2026-03-18T11:13:38.240-05:00 DEBUG 15220 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.QUERY                : Executing query: SELECT p.id, p.description, p.price
FROM products AS p
    INNER JOIN customer_orders AS co ON(p.id = co.product_id)
    INNER JOIN customers AS c ON(co.customer_id = c.id)
WHERE c.name = $1

2026-03-18T11:13:38.272-05:00  INFO 15220 --- [webflux-playground] [actor-tcp-nio-3] d.m.a.s.Lec03CustomerOrderRepositoryTest : Product(id=1, description=iphone 20, price=1000)
2026-03-18T11:13:38.323-05:00  INFO 15220 --- [webflux-playground] [actor-tcp-nio-3] d.m.a.s.Lec03CustomerOrderRepositoryTest : Product(id=4, description=mac pro, price=3000)
````

### ✅ Conclusión

- `@Query` permite definir consultas SQL personalizadas directamente en el repositorio.
- Es ideal para consultas que involucran múltiples tablas y relaciones `(JOINs)`.
- El test confirma que la consulta retorna correctamente los productos asociados a un cliente específico.

### 🧩 Ejemplo 2: Consultas complejas con `@Query` y `projections` (record de Java 17)

Cuando el resultado de una consulta devuelve solo un subconjunto de columnas
—`ya sea desde una única tabla o desde múltiples tablas mediante JOINs`—, en lugar de mapear el resultado a una
entidad completa, podemos usar un `record` o una `interfaz` para representarlo como un `DTO`.

Esto permite tener estructuras más ligeras, expresivas y alineadas con los datos realmente necesarios en cada caso.

### 🧩 Definición del record `OrderDetails`
...
Definimos un método en el repositorio que retorna directamente la proyección `OrderDetails`:
...
> `Spring Data` mapea automáticamente el resultado de la consulta al record porque los alias de las columnas
> (`customer_name`, `product_name`, etc.) coinciden con los nombres de los campos del `DTO`.
...
El log muestra la consulta ejecutada y los resultados mapeados al record:

````bash
2026-03-18T12:00:30.942-05:00 DEBUG 572 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.PARAM                : Bind parameter [0] to: iphone 18
2026-03-18T12:00:30.969-05:00 DEBUG 572 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.QUERY                : Executing query: SELECT co.order_id,
        c.name AS customer_name,
        p.description AS product_name,
        co.amount,
        co.order_date
...
2026-03-18T12:00:31.003-05:00  INFO 572 --- [webflux-playground] [actor-tcp-nio-3] d.m.a.s.Lec03CustomerOrderRepositoryTest : OrderDetails[orderId=a243c1d3-d3dc-495a-a2ec-c587c90405e5, customerName=sam, productName=iphone 18, amount=850, orderDate=2026-03-18T17:00:29.067953Z]
2026-03-18T12:00:31.052-05:00  INFO 572 --- [webflux-playground] [actor-tcp-nio-3] d.m.a.s.Lec03CustomerOrderRepositoryTest : OrderDetails[orderId=fe7cf524-57f2-477c-9346-fa7bae8fc98d, customerName=jake, productName=iphone 18, amount=775, orderDate=2026-03-18T17:00:29.073156Z]
2026-03-18T12:00:31.053-05:00  INFO 572 --- [webflux-playground] [actor-tcp-nio-3] d.m.a.s.Lec03CustomerOrderRepositoryTest : OrderDetails[orderId=703d3c5a-d019-4e28-9b5e-bad4d25d066a, customerName=jake, productName=iphone 18, amount=750, orderDate=2026-03-18T17:00:29.073156Z] 
````

### ✅ Conclusión

- Los `records de Java 17` son una forma concisa y expresiva de definir proyecciones en consultas complejas.
- `Spring Data R2DBC` mapea automáticamente las columnas al DTO si los nombres coinciden con los alias definidos
  en la consulta.
- Esto permite trabajar con estructuras más ligeras que las entidades completas, optimizando el rendimiento y la
  claridad del código.

### 2️⃣ Utilizando el R2DBC `DatabaseClient` para ejecutar consultas de manera programática

Cuando necesitamos mayor control sobre nuestras consultas SQL —por ejemplo, para realizar queries dinámicas, complejas o
no triviales que involucren `JOINs`, `subconsultas`, `funciones agregadas`, etc.— el `DatabaseClient` de **Spring Data
R2DBC** es una herramienta poderosa.

A diferencia de los repositorios reactivos (`ReactiveCrudRepository`), con `DatabaseClient` no estamos limitados a
métodos predefinidos o anotaciones como `@Query`. Aquí escribimos SQL de forma libre y podemos mapear los resultados a
cualquier clase `DTO`, `record` o entidad.

#### 📌 ¿Cuándo usar `DatabaseClient`?

- Cuando necesitas construir consultas SQL **complejas o dinámicas**.
- Cuando no deseas (o no puedes) usar `@Query` por alguna limitación.
- Cuando quieres mapear resultados a clases personalizadas que no son entidades (como `DTOs` o `records`).
- Para operaciones más generales que no encajan bien en la abstracción de un repositorio.

#### ✅ Ventajas del uso de `DatabaseClient`

- Permite trabajar directamente con **SQL nativo**.
- Ideal para escenarios en los que necesitas **máxima flexibilidad**.
- Compatible con mapeo automático a `POJOs`, `records` y entidades.
- Útil en **tests** y scripts de inicialización.

### 🧪 Ejemplo: Consulta con `DatabaseClient`

En este ejemplo realizamos una consulta que une tres tablas (`customers`, `customer_orders`, y `products`) para obtener
los detalles de las órdenes que coinciden con un producto específico. El resultado se mapea directamente al `record`
`OrderDetails`.
...
El log muestra la consulta ejecutada y los resultados obtenidos:

````bash
2026-03-18T12:18:38.844-05:00 DEBUG 16120 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.PARAM                : Bind parameter [0] to: In{Inferred: java.lang.String}
2026-03-18T12:18:38.895-05:00 DEBUG 16120 --- [webflux-playground] [actor-tcp-nio-3] io.r2dbc.postgresql.QUERY                : Executing query: SELECT co.order_id,
        c.name AS customer_name,
        p.description AS product_name,
        co.amount,
        co.order_date
...
2026-03-18T12:18:38.913-05:00  INFO 16120 --- [webflux-playground] [actor-tcp-nio-3] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=56a2e400-7138-4e35-93fd-14d97c695c03, customerName=sam, productName=iphone 18, amount=850, orderDate=2026-03-18T17:18:37.197686Z]
2026-03-18T12:18:38.966-05:00  INFO 16120 --- [webflux-playground] [actor-tcp-nio-3] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=d9c3550c-a29d-4523-854f-a13120df8079, customerName=jake, productName=iphone 18, amount=775, orderDate=2026-03-18T17:18:37.202562Z]
2026-03-18T12:18:38.967-05:00  INFO 16120 --- [webflux-playground] [actor-tcp-nio-3] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=453edbdb-c47c-4efd-a159-d1ed23c9d0ab, customerName=jake, productName=iphone 18, amount=750, orderDate=2026-03-18T17:18:37.202562Z]
````

### ✅ Conclusión

- `DatabaseClient` ofrece máxima flexibilidad para consultas SQL en aplicaciones reactivas.
- Permite mapear resultados directamente a records o DTOs, sin necesidad de entidades completas.
- Es ideal para consultas complejas que involucran múltiples tablas y condiciones avanzadas.
- El test confirma que la consulta se ejecuta correctamente y los resultados se mapean automáticamente al record
  `OrderDetails`.

## 🧪 Ejecutando test de la `sec03` desde cmd

Para ejecutar únicamente los test de la **sección 03**, nos posicionamos en la raíz del proyecto `webflux-playground` y
...
[INFO] --------------------------------[ jar ]---------------------------------
[INFO]
[INFO] --- resources:3.3.1:resources (default-resources) @ webflux-playground ---
[INFO] Copying 1 resource from src\main\resources to target\classes
[INFO] Copying 2 resources from src\main\resources to target\classes
[INFO]
[INFO] --- compiler:3.14.1:compile (default-compile) @ webflux-playground ---
[INFO] Nothing to compile - all classes are up to date.
...
2026-03-24T12:50:15.418-05:00  INFO 5640 --- [webflux-playground] [actor-tcp-nio-1] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=bded4b18-acb2-45b9-866d-d46b7cc12ddc, customerName=sam, productName=iphone 18, amount=850, orderDate=2026-03-24T17:50:13.785297Z]
2026-03-24T12:50:15.418-05:00  INFO 5640 --- [webflux-playground] [actor-tcp-nio-1] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=4d0d4d2c-8819-42b8-a721-9b0e5024b3e3, customerName=jake, productName=iphone 18, amount=775, orderDate=2026-03-24T17:50:13.789205Z]
2026-03-24T12:50:15.419-05:00  INFO 5640 --- [webflux-playground] [actor-tcp-nio-1] d.m.app.sec03.Lec04DatabaseClientTest    : OrderDetails[orderId=d22d1963-cf8f-413e-8e44-ca886868ef20, customerName=jake, productName=iphone 18, amount=750, orderDate=2026-03-24T17:50:13.789205Z]
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.028 s -- in dev.magadiflo.app.sec03.Lec04DatabaseClientTest
[INFO]
[INFO] Results:
[INFO]
...
[INFO] Finished at: 2026-03-24T12:50:15-05:00
[INFO] ------------------------------------------------------------------------
````
...
- `-Dtest="*/sec03/*"` → bandera que indica a Maven que solo ejecute los test que se encuentran en el paquete o
  directorio `sec03`.
    - El patrón `*/sec03/*` permite filtrar las clases de prueba por ubicación.
    - Esto es útil cuando queremos correr únicamente los test de una sección específica del curso o módulo del proyecto.

### ✅ Conclusión

- Con este comando podemos ejecutar de forma selectiva los test de la `sección 03`, sin necesidad de correr todo el
  proyecto.
- La salida confirma que se ejecutaron correctamente 11 test (Failures: 0, Errors: 0).
- Esto garantiza que los ejemplos de la `sección 03` funcionan correctamente y están validados con pruebas de
  integración.
