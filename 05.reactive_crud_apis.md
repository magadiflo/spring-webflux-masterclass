...
Aquí aplicamos los métodos HTTP estándar (`GET`, `POST`, `PUT`, `DELETE`) y aprovechamos la naturaleza
**no bloqueante** de WebFlux.
...
## 📌 `Mono.just` vs `Mono.fromSupplier` en Spring WebFlux

En este apartado explicaré la razón de por qué en el método `allCustomers()` he usado el
` return Mono.just(ResponseEntity.ok(customerResponseFlux));` en lugar de
` return Mono.fromSupplier(() -> ResponseEntity.ok(customerResponseFlux));`

### 🔎 ¿Cuándo importa la diferencia?

La diferencia entre `Mono.just(...)` y `Mono.fromSupplier(...)` es relevante **únicamente** cuando el mismo operador
es compartido entre múltiples suscriptores, como por ejemplo un atributo de clase:
...
public class MyService {
    ...
    // ⚡ Mono.just → eager: el valor se calcula UNA SOLA VEZ al crear el bean
    // Cada suscriptor siempre recibirá el mismo UUID generado al levantar la aplicación
    private final Mono<String> mono = Mono.just(calcularAlgoCostoso());

    // 🦥 Mono.fromSupplier → lazy: el valor se calcula en cada suscripción
    // Cada suscriptor recibirá un UUID distinto
    private final Mono<String> mono2 = Mono.fromSupplier(() -> calcularAlgoCostoso());

    public Mono<String> getValorCompartido() {
        return this.mono; // todos los suscriptores recibirán el mismo UUID
    }

    public Mono<String> getValorPorSuscripcion() {
        return this.mono2; // cada suscriptor recibirá un UUID distinto
    }

    private String calcularAlgoCostoso() {
        return UUID.randomUUID().toString();
    }
}
```

En este caso sí importa cuál usar, porque el mismo `Mono` es compartido y reutilizado entre múltiples suscriptores.

### 🌐 Dentro de un método: la diferencia se vuelve irrelevante

Cuando `Mono.just(...)` o `Mono.fromSupplier(...)` se usan dentro de un método, **ambos viven y mueren con el
contexto de ese método**. Cada invocación del método crea su propio operador, por lo que no hay riesgo de estado
compartido entre invocaciones.

```java
// Ambos son equivalentes dentro de un método
public Mono<ResponseEntity<Flux<CustomerResponse>>> allCustomers() {
    Flux<CustomerResponse> flux = this.customerService.getAllCustomers();
    return Mono.just(ResponseEntity.ok(flux));                  // ✅ correcto
    return Mono.fromSupplier(() -> ResponseEntity.ok(flux));    // ✅ también correcto, pero innecesario
}
```

Esto aplica no solo a endpoints, sino a **cualquier método** que retorne un `Mono.just(...)` o
`Mono.fromSupplier(...)`. Si llegan 100 peticiones HTTP al método `allCustomers()`, cada una tendrá su propio
contexto del método y por lo tanto su propio operador.

> 💡 La única razón válida para usar `Mono.fromSupplier(...)` dentro de un método es **semántica**: comunicar que
> hay un cálculo costoso o lazy. Si no hay cálculo costoso, `Mono.just(...)` es la elección correcta por claridad
> y simplicidad.

### 📋 Resumen

| Escenario                                              | Operador recomendado | Motivo                                          |
|--------------------------------------------------------|----------------------|-------------------------------------------------|
| Atributo de clase, valor costoso de calcular           | `Mono.fromSupplier`  | Lazy evaluation, se calcula en cada suscripción |
| Atributo de clase, valor simple                        | `Mono.just`          | Eager, el valor se calcula una sola vez         |
| Dentro de un método, valor simple                      | `Mono.just`          | Vive en el contexto del método, sin riesgo      |
| Dentro de un método, cálculo costoso o llamada externa | `Mono.fromSupplier`  | Comunica intención de lazy evaluation           |
