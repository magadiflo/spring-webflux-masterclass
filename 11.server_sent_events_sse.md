...
- `.map(this.productMapper::toProduct)`: convierte el DTO en entidad de base de datos.
- `.flatMap(this.productRepository::save)`: guarda la entidad en la base de datos de forma reactiva.
- `.doOnNext(product -> log.info(...))`: imprime en logs cada producto guardado.
